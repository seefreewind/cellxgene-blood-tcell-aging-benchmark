...
def build_docx() -> None:
    ...
    p(doc, "Background: Public single-cell atlases now provide reusable embeddings for prediction tasks. These resources also create a validation problem. Random-cell splits can overestimate generalizable performance when age labels track donor identity, disease status, assay, or study origin.")
    p(doc, f"Methods: We built a leakage-aware benchmark for CELLxGENE-hosted public single-cell resources. After auditing CELLxGENE Census release {meta_summary['census_version']}, we completed the empirical benchmark in blood T cells. Young and old donors were defined from development_stage-derived numeric ages of <=40 and >=60 years. The benchmark included {int(bench_audit['sample_all_donors'])} balanced donors and {int(bench_audit['sample_all_cells'])} sampled cells. A disease-free subset included {int(bench_audit['sample_disease_free_donors'])} donors and {int(bench_audit['sample_disease_free_cells'])} cells. We evaluated hosted scVI and TranscriptFormer TF-Sapiens embeddings, donor-mean embeddings, and a metadata-only baseline across 30 repeated seeds.")
    p(doc, f"Results: Random-cell AUROC in blood T cells was {metric_ci('metadata_confounder_logistic','random_cell')} for metadata alone, {metric_ci('TranscriptFormer_embedding_logistic','random_cell')} for TranscriptFormer, and {metric_ci('scVI_embedding_logistic','random_cell')} for scVI. Thus, metadata-only prediction exceeded or matched the hosted embedding models under the easiest split. Dataset-holdout AUROC was lower and more variable: {metric_ci('metadata_confounder_logistic','dataset_holdout')} for metadata, {metric_ci('TranscriptFormer_embedding_logistic','dataset_holdout')} for TranscriptFormer, and {metric_ci('scVI_embedding_logistic','dataset_holdout')} for scVI. Disease-free donor-holdout performance was strongest for donor-mean embeddings, reaching {metric_ci('TranscriptFormer_mean_embedding_pseudobulk','disease_free_donor_holdout')} for TranscriptFormer and {metric_ci('scVI_mean_embedding_pseudobulk','disease_free_donor_holdout')} for scVI. LODO evaluation was feasible for one held-out dataset, showing limited cross-study validation capacity in the available metadata.")
    p(doc, "Conclusions: This benchmark shows that random-cell evaluation can overstate generalizable blood T-cell aging prediction in public CELLxGENE data. Donor-, disease-, and dataset-aware diagnostics are needed before public single-cell age-prediction results are interpreted biologically.")
    ...
    p(doc, "Public single-cell atlases are now used not only as biological references, but also as benchmark resources. The Human Cell Atlas, Human Cell Landscape, and Tabula Sapiens established shared infrastructures for cross-tissue and cross-study analysis [14-16]. CZ CELLxGENE Discover extends this infrastructure by aggregating public datasets and exposing hosted resources, including embeddings that can be used without retraining each model from raw counts [17].")
    p(doc, "Aging prediction is a useful stress test for these hosted representations. Molecular clocks showed that age can be estimated from high-dimensional molecular profiles [1,2]. Single-cell aging studies then showed that age-associated structure can vary by tissue and cell type [7-9]. Other human and multi-tissue resources support the same cell-type-aware framing [10-12]. In public atlases, however, age labels are observational metadata. Prediction performance can reflect how donors, studies, assays, and diseases are distributed, not only cellular aging biology.")
    p(doc, "This distinction matters when age is parsed from public metadata fields. Age groups may be unevenly distributed across recruitment settings, assay types, datasets, diseases, and donor attributes. Best-practice guidance and integration methods show that preprocessing and latent representations can shape downstream analyses [25-27]. Batch effects can dominate benchmark behavior if they are not measured directly [28-30]. Atlas composition creates a related source of benchmark instability [31].")
    p(doc, "Representation models make this validation problem more pressing. scVI introduced a probabilistic latent-variable framework for single-cell expression [18,19]. Transcriptomic representation models such as Geneformer, scGPT, and scFoundation have expanded the set of reusable cellular embeddings [20-22]. GeneCompass and TranscriptFormer further illustrate the move toward pretrained transcriptomic representations [23,24]. These models can encode biological structure, but they can also retain study, assay, and cohort structure. This study directly evaluates CELLxGENE-hosted scVI and TranscriptFormer embeddings.")
    p(doc, "Previous single-cell aging atlases and representation-model studies have not provided a donor-, disease-, and study-aware benchmark for hosted public embeddings. A useful benchmark should include metadata-only baselines, donor-aware splitting, disease-free sensitivity analysis, grouped dataset-holdout evaluation, label-shuffling controls, LODO feasibility diagnostics, and sampling-depth sensitivity. These design choices follow concerns about leakage and small-sample validation [39,40]. They also draw on prediction-model reporting and external-validation guidance [41-43]. Clinical AI evaluation supports conservative interpretation of public-data prediction results [44,45]. Dataset-shift and causal-interpretation limits further motivate source-aware benchmarking [46,47].")
    p(doc, "We therefore developed a leakage-aware benchmark for hosted single-cell embeddings in public aging prediction. Blood T cells are the completed empirical stratum. The aim is a reproducible validation framework, not a pan-tissue aging predictor.")
    ...
    p(doc, f"Threshold sensitivity showed that eligibility depended on donor-count and disease-free retention criteria. The primary threshold yielded {int(base['eligible_strata_n'])} eligible strata. A stricter rule requiring at least 40 donors and disease-free retention of 0.60 yielded {int(strict['eligible_strata_n'])} eligible strata.")
    ...
    p(doc, f"In blood T cells, random-cell AUROC was {metric_ci('metadata_confounder_logistic','random_cell')} for metadata alone, {metric_ci('TranscriptFormer_embedding_logistic','random_cell')} for TranscriptFormer, and {metric_ci('scVI_embedding_logistic','random_cell')} for scVI. Metadata-only random-cell performance exceeded both cell-level embedding models. Dataset, assay, disease, and sex carried substantial age-predictive structure in the sampled public data.")
    ...
    p(doc, f"Donor-holdout AUROC was lower than or comparable to random-cell performance for cell-level embeddings. It reached {metric_ci('TranscriptFormer_embedding_logistic','donor_holdout')} for TranscriptFormer and {metric_ci('scVI_embedding_logistic','donor_holdout')} for scVI. Donor-mean embeddings increased donor-holdout AUROC to {metric_ci('TranscriptFormer_mean_embedding_pseudobulk','donor_holdout')} for TranscriptFormer and {metric_ci('scVI_mean_embedding_pseudobulk','donor_holdout')} for scVI.")
    p(doc, f"Dataset-holdout AUROC was lower and more variable. It was {metric_ci('metadata_confounder_logistic','dataset_holdout')} for metadata alone, {metric_ci('TranscriptFormer_embedding_logistic','dataset_holdout')} for TranscriptFormer, and {metric_ci('scVI_embedding_logistic','dataset_holdout')} for scVI. Apparent aging prediction was sensitive to study composition and dataset shift.")
    ...
    p(doc, "Age labels were unevenly distributed across dataset, disease, assay, and sex. The metadata-only coefficient analysis identified source and assay categories with large old-versus-young coefficients. Embedding principal components also aligned visibly with non-age metadata. These diagnostics support including metadata baselines in public-data benchmarks.")
    ...
    if not lodo.empty:
        ...
        p(doc, f"Only one dataset met LODO eligibility after requiring both age classes in the held-out set. This dataset contained {int(best['heldout_donors'])} donors and {int(best['heldout_cells'])} cells. Across the eligible LODO test, AUROC ranged from {lodo['auroc'].min():.3f} to {lodo['auroc'].max():.3f} across models. The highest value was observed for {best['model']}. The remaining {len(lodo_excl)} datasets were excluded because they lacked sufficient young and old donors. LODO feasibility was itself a diagnostic result, showing limited cross-study validation capacity in this stratum.")
    else:
        p(doc, "No dataset met LODO eligibility after requiring both age classes in the held-out set. All exclusions were recorded.")
    p(doc, f"In the disease-free donor-holdout subset, cell-level AUROC was {metric_ci('TranscriptFormer_embedding_logistic','disease_free_donor_holdout')} for TranscriptFormer and {metric_ci('scVI_embedding_logistic','disease_free_donor_holdout')} for scVI. Donor-mean embeddings increased performance to {metric_ci('TranscriptFormer_mean_embedding_pseudobulk','disease_free_donor_holdout')} and {metric_ci('scVI_mean_embedding_pseudobulk','disease_free_donor_holdout')}, respectively. This result describes benchmark behavior after disease-status restriction within the available public metadata.")
    p(doc, "High disease-free donor-holdout AUROC did not remove residual dataset, assay, recruitment, or cohort effects. Disease-free filtering reduced one observed source of confounding. Donors could still be structured by study origin, technical protocol, source population, and metadata completeness.")
